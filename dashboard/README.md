...
The items below include implementation notes so new contributors can ship them quickly. Each one keeps the new JSON data ingestion flow as the single source of truth.

- [x] **Load real data from JSON (generated by `run_analysis.py`)**
  - Status: Completed. Data is fetched at runtime from `public/dashboard-data.json` with a fallback to `src/data/dashboardData.js` for local development.

- [ ] **Add export functionality (PDF, Excel, CSV)**
  - Approach: Add an `ExportMenu` component under `src/components/` that reads the in-memory dataset from `useDashboardData`. Use [`exceljs`](https://www.npmjs.com/package/exceljs) for XLSX, `FileSaver` for CSV/JSON blobs, and `html2canvas` + `jspdf` for PDF snapshots of the active tab.
  - UX: Place a toolbar in `App.jsx` so exports are consistent across tabs. Show a toast on success/failure.

- [ ] **Implement data filtering and search**
  - Approach: Add a shared `filters` slice via React context (e.g., `DashboardFiltersProvider` in `src/context/`). Include text search, tenure, borough, and pathway selectors. Feed the filter state into each section component before rendering charts.
  - UI: Add a compact filter bar above the tab content with clear/reset controls; persist defaults in `localStorage`.

- [ ] **Add user preferences and saved views**
  - Approach: Store preferences (selected tab, filters, theme) in `localStorage` with a `usePreferences` hook under `src/data/`. Add a “Save view” action that serializes the current state and provides a shareable URL query string (e.g., `?tab=pathways&borough=Camden`).
  - UX: Surface a “Saved views” dropdown in the header to quickly restore presets.

- [ ] **Create print-optimized layouts**
  - Approach: Add print styles in `src/index.css` using `@media print` to simplify colors, hide navigation, and collapse cards into a single column. Provide a “Print this tab” button that triggers `window.print()` after applying a `print-mode` class.
  - Testing: Verify charts render legibly on A4 with default margins.

- [ ] **Add animation for chart transitions**
  - Approach: Enable Recharts animation props (`isAnimationActive`, `animationDuration`) for key chart types and gate them behind a `prefers-reduced-motion` check. Centralize defaults in a `chartConfig` helper.
  - UX: Keep durations under 400ms to preserve readability.

- [ ] **Implement dark mode theme**
  - Approach: Introduce CSS variables in `src/index.css` and toggle a `data-theme` attribute on the `<body>` (defaulting to system preference via `matchMedia`). Expose a theme switcher in the header and persist the choice with `usePreferences`.
  - Visuals: Adjust chart color palettes to maintain contrast in both themes.

- [ ] **Add comparison mode for scenarios**
  - Approach: Add a “Compare” drawer that lets users select up to three pathways or scenarios. Reuse existing chart components but feed them sliced datasets; show a small-multiples grid with synchronized tooltips.
  - Data: Extend `dashboard-data.json` to include scenario keys if not already present.

- [ ] **Include interactive map integration**
  - Approach: Add a `MapView` component (e.g., using `react-leaflet`) under `src/components/` to display borough-level or LSOA-level aggregates. Load geojson boundaries from `public/` and join them to the analysis outputs.
  - Performance: Lazy-load the map tab to avoid impacting the core dashboard bundle.

- [ ] **Add detailed property-level drill-down**
  - Approach: Provide a modal or side panel that displays property-level metrics when a chart element is clicked. Fetch the drill-down data from a paginated endpoint or a pre-sliced JSON (e.g., `public/properties/{borough}.json`) generated alongside `dashboard-data.json`.
  - UX: Include breadcrumb navigation back to aggregated views and guard sensitive fields before export/print.
