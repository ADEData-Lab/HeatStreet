...
- [x] **Live data ingestion** – Runtime fetch of `public/dashboard-data.json` with a fallback to bundled defaults for offline use.
- [x] **Export functionality (JSON, CSV, Excel, PDF)** – `ExportMenu` uses FileSaver, ExcelJS, and html2canvas + jsPDF to export the current dataset in multiple formats.
- [x] **Data filtering and search** – A global filter bar (search, borough, pathway) is backed by the shared dashboard context so every chart/table respects the active filters.
- [x] **User preferences and saved views** – Theme and animation choices persist in `localStorage`; users can save and reload named views that capture filters and the active tab.
- [x] **Print-optimized layouts** – `@media print` styles simplify the UI for A4 printing, and the header includes a one-click “Print this view” action.
- [x] **Animation controls** – Recharts animations are enabled by default but can be toggled off through preferences for reduced-motion users.
- [x] **Dark mode theme** – CSS variables power a light/dark theme toggle, respecting system preference when set to “System.”
- [x] **Scenario comparison mode** – A comparison drawer lets users pin up to three scenarios for side-by-side cost/CO₂ charts.
- [x] **Interactive map integration** – The borough tab now includes a Leaflet map that joins `dashboard-data.json` metrics to geocoded borough points for quick spatial inspection.
- [x] **Drill-down overlays** – Clicking scenario bars, borough rows, or map markers opens a detail modal showing the underlying record.
